...
//		check for file write/read
		file_put_contents(EZPZOCB . "/compatability-test.$dataKey.php", 'is_compatible' );
		if(file_get_contents(EZPZOCB . "/compatability-test.$dataKey.php") != 'is_compatible'){
			$failed[] = '<p>EZPZ OCB cannot properly write and/or read new files.';
		} else {
			unlink(EZPZOCB . "/compatability-test.$dataKey.php");
		}
...
function ezpz_control_bar() {
	...
	$dl_load = EZPZOCB_URL . "/backups/data/backup-list.$dataKey.php";
...
function post_num($output) {
	...
	file_put_contents(EZPZOCB . "/backups/data/buNum.$dataKey.php", $output);
	return $output;
}
...
function get_sql_dump($sql_file, $display = false) {
	...
	if (get_option('ezpz_ocb_db_dump') != "alt") {// Primary database backup method
		...
		if ($prefix_only === "yes") {
			...
			$command = 'mysqldump -p --user="' . base64_decode($db_user) . '" --password="' . base64_decode($db_password) . '" --host="' . base64_decode($db_host) . '" --opt "' . base64_decode($db_name) . '" ' . $tablelist . ' > "' . $sql_file . '"';
		} else {
			//        Entire database backup command
			$command = 'mysqldump -p --user="' . base64_decode($db_user) . '" --password="' . base64_decode($db_password) . '" --host="' . base64_decode($db_host) . '" --add-drop-table "' . base64_decode($db_name) . '" > "' . $sql_file . '"';
		}
		...
		db_dump(base64_decode($db_host), base64_decode($db_user), base64_decode($db_password), base64_decode($db_name), $sql_file);
	}
	usleep(250000);
}
...
function rename_backup_folder($name) {
	...
	$oldDirectory = file_get_contents(EZPZOCB . "/backups/data/directory.$dataKey.php");
	$newDirectory = str_replace(get_option('ezpz_ocb_backup_folder_name'), $name, $oldDirectory);
	file_put_contents(EZPZOCB . "/backups/data/directory.$dataKey.php", $newDirectory);
...
function list_backups() {
	...
	if ($handle = opendir(ezpz_ocb_backup_folder())) {
		...
		rsort($backup_list);
		$backup_list[] .= "\n</ul>";
		file_put_contents(EZPZOCB . "/backups/data/backup-list.$dataKey.php", $backup_list);
		if ($buCnt == 0) {
			file_put_contents(EZPZOCB . "/backups/data/directory.$dataKey.php", '');
		}
	}
}
...
function manage_backups($zip_file = '') {
	if (file_exists($zip_file) || $zip_file == '') {
		...
		if ($logCnt != 0) {
			...
			global $dataKey;
			file_put_contents(EZPZOCB . "/backups/data/directory.$dataKey.php", $start . $temp2 . $end);
		}
	}
...
function get_status($content) {
	...
	if ($last === "?") {
		...
		if (file_get_contents(EZPZOCB . "/backups/data/" . $first . "-status.$dataKey.php") != '') {
			return true;
		} else {
			return false;
		}
	} else {
		return EZPZOCB_URL . "/backups/data/" . $content . "-status.$dataKey.php?";
	}
}
...
function set_status($type, $content) {
	...
	if($type == 'abort'){
		file_put_contents(EZPZOCB . "/backups/data/backup-status.$dataKey.php", $content);
	}

	if ($content !== 'unset') {
		file_put_contents(EZPZOCB . "/backups/data/" . $type . "-status.$dataKey.php", $content);
	} else {
		if (file_exists(EZPZOCB . "/backups/data/" . $type . "-status.$dataKey.php")) {
			unlink(EZPZOCB . "/backups/data/" . $type . "-status.$dataKey.php");
		}
	}
}
...
//	if($array['type'] != 'manual'){				
		...
		sleep(7);
/*	} else {
		$error_message = ezpz_ocb_sp() . '&aborted=' . urlencode(get_option('ezpz_ocb_zip_name') . ' Aborted!');
...
function reset_ezpzocb(){
	...
	update_option('ezpz_ocb_backup_type', 'none');
	file_put_contents(EZPZOCB . "/backups/data/directory.$dataKey.php", "Backup's cleared on " . wp_date());
}
